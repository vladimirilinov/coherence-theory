...
\[
\mathrm{CL}(A,\Phi)=\sup_{T\in\mathscr T} F_T(A,\Phi),\qquadd
...
>
> \[
> \langle X,Y\rangle_{2;\Lambda}:=\frac{1}{d_\Lambda}\,\mathrm{tr}_\Lambda(X^\dagger Y),\qquadd 
...
6. **(H5) Complete CP-monotonicity for leakage (sub-Markov in \$W\$).** For any admissible block-local CP maps \$\Phi\_{\rm pre},\Phi\_{\rm post}\$ whose **HS-adjoints** satisfy
   ...
   \[
\Phi_{\rm pre}^{\!}(W)\le W,\qquadd \Phi_{\rm post}^{\!}(W)\le W,
\]
...
7. **(H5.lin) Weight linearity (Dirichlet linearity postulate).** For all \$\alpha\ge 0\$ and positive \$W\_1,W\_2\$ with \$W\_1W\_2=0\$,
   ...
   \[
\mathcal B_{\rm leak}^\Lambda(\{L_j\};\alpha W)=\alpha\,\mathcal B_{\rm leak}^\Lambda(\{L_j\};W),\qquadd
...
\[
\mathcal B_{\rm th}=\sup_\Lambda \mathcal B_{\rm th}^\Lambda,\qquadd
\mathcal B_{\rm cx}=\sup_\Lambda \mathcal B_{\rm cx}^\Lambda,\qquadd
...
\[
\boxed{\ \mathcal B=\alpha_{\rm th}\,\mathcal B_{\rm th}\;+\;\alpha_{\rm cx}\,\mathcal B_{\rm cx}\;+\;\alpha_{\rm leak}\,\mathcal B_{\rm leak}\;+\;\mathcal N,\qquadd \alpha_\bullet\ge0,\ \ \mathcal N\in\mathsf N\ .}
\]
...
\[
\delta_H(A):=i[H,A],\qquadd A\in\mathcal A_{\rm loc}.
\]
...
\[
\langle X,Y\rangle_{\rm HS;\Lambda}:=\frac{1}{d_\Lambda}\operatorname{Tr}(X^\dagger Y),\qquadd 
...
**Leakage budget.** For a Kraus list \${L\_j}\$ and a positive affiliated weight \$W\succ0\$ in the GNS von Neumann algebra,
...
[
\dot U_t=-\,iH_tU_t,\qquadd A_t:=U_t^\dagger A\,U_t,\qquadd \dot A_t=i[H_t,A_t].
\]

**Variations.** Admissible variations are \$\delta U\_t=-,iK\_t U\_t\$ with \$K\_t^\dagger=K\_t\$ and \$K\in C\_c^1((0,T))\$; then

\[
\delta A_t=i[K_t,A_t],\qquadd \delta H_t=\dot K_t+i[K_t,H_t].
\]
...
\[
\boxed{\ \mathcal A_A:=\mathrm{ad}_A^{\!*}\mathrm{ad}_A=[A,[A,\cdot]]\ \ge 0,\qquadd \mathrm{ad}_A(\cdot):=[A,\cdot].\ }
\]
...
[
\boxed{\ \dot A_t\ =\ \frac{i}{\hbar}\,[H^\*(A_t),A_t],\qquadd
...
\[
\boxed{\ \mathcal A_A\,H^\*(A)\ =\ \frac{-\,i}{\lambda_{\rm th}}\,\big[A,\,G(A)\big],\qquadd H^\*(A)\ \perp\ \ker\mathcal A_A. }
\]
...
Rewrite \$\langle \mathcal A\_A H,[K,H]\rangle\_{\rm HS}=\langle[H,\mathcal A\_A H],K\rangle\_{\rm HS}\$ to obtain
...
* Pricing the **orbit speed** \$|[H,A]|\_{\rm HS}\$ (rather than the Ad-invariant superoperator norm) and matching the **same** normalized HS geometry for gradient and quadratic yields
  ...
  [
\dot A=\frac{i}{\hbar}[H^\*(A),A],\qquadd
...
$\lambda\,|\xi|^4\ \le\ \sigma(\mathbb A_\varepsilon(g))[\xi]\ \le\ \Lambda\,|\xi|^4\qquaddd\text{(parameter‑ellipticity on compacts).}$
...
\[
\langle T^{\rm eff},\delta g\rangle\ \le\ V_g^\circ(g,A;\delta g),\qquadd
...
\[
V_g^\circ(g,A;\mathcal L_X g)=V_g^\circ(g,A;-\mathcal L_X g)=0,\qquadd
...
\[
\langle T^{\rm eff},\mathcal L_X g\rangle=0,\qquadd \langle J^{\rm eff},D\chi\rangle=0.
\]

Integrating by parts within the cone-class yields the weak conservation laws

\[
\nabla^\mu T^{\rm eff}_{\mu\nu}=0,\qquadd D^\mu J^{\rm eff}_\mu=0
\]
...
\[
T_U=\frac{\kappa}{2\pi},\qquadd \beta_U:=T_U^{-1}.
\]
...
\[
\tilde\gamma_k^{\rm H}=\gamma_k^{\rm H}\,e^{-\beta_U\omega_k},\qquadd \bar n_k=(e^{\beta_U\omega_k}-1)^{-1}.
\]
...
\[
\boxed{\
I(A:\bar A)\ \le\ \chi_\*(\beta_U,{\rm LR},W)\,\frac{{\rm Area}(A)}{\ell_\*^{\,d-2}}\ +\ O(|\partial A|),\qquadd
...
\end{cases}
\qquadd 0<v_k\le v_{\max}.
\]

For a tile \$T\$,

\[
\mathcal F_{\rm out}(T)=\sum_{k\in\mathcal K_T} c_k\,\Gamma_k,\qquadd 
\tau_{\rm leak}(T)=\sum_{k\in\mathcal K_T} w_k\,\Gamma_k.
\]
...
\[
\tau_{\rm H}(T):=\sum_{k\in\mathcal K_T} w_k^{\rm H}\,\Gamma_k^{\rm H},\qquadd
...
\[
C_k(t):=\langle b_k^{\rm out}(t)\,(b_k^{\rm out})^\dagger(0)\rangle,\qquadd 
...
\[
C_k(t)=\big(1+\bar n_k\big)\,e^{-\tfrac12\Gamma_k t},\qquadd 
...
\[
H[N]:=\int_\Sigma\! N\,\mathcal H_\perp,\qquadd D[\vec N]:=\int_\Sigma\! N^a\,\mathcal H_a.
\]
...
[
\{q_{ab},D[\vec N]\}=\mathcal L_{\vec N}q_{ab},\qquadd \{\pi^{ab},D[\vec N]\}=\mathcal L_{\vec N}\pi^{ab}.
\]
...
[
T[N]=\int N\ \frac{1}{\sqrt q}\Big(\pi^{ab}\pi_{ab}-\tfrac12\pi^2\Big),\qquadd
...
\[
\frac{\delta T[N]}{\delta \pi^{ab}}=\frac{2N}{\sqrt q}\Big(\pi_{ab}-\tfrac12\pi\,q_{ab}\Big),\qquadd
...
[
\frac{\delta V[N]}{\delta q_{ab}}= -N\sqrt q\,(G^{ab}+\Lambda q^{ab}) + \text{(total divergences)},\qquadd \frac{\delta V[N]}{\delta \pi^{ab}}=0.
\]
...
\[
\{H[N],H[M]\}=D[\vec\beta],\qquadd \vec\beta=\beta^a\partial_a.
\]
...
\[
\mu_P(\mathrm d x):=\frac{1}{Z_P}\,\exp\!\big(\!-S_P(x)\big)\,\mathbf 1_{\text{single-cone}}(x)\ \nu_P(\mathrm d x),\qquadd Z_P:=\int e^{-S_P}\mathbf 1_{\text{cone}}\ \mathrm d\nu_P.
\tag{9.3}
\]

### Cylinder consistency (projective system)

If \$P\preceq P'\$ (refinement), write \$\pi\_{P'!,P}:\mathcal X\_{P'}!\to\mathcal X\_P\$ for restriction/averaging on cells. We define \$S\_P\$ by *backward induction* from fine to coarse scales:
...
\[
(\pi_{P'\!,P})_\#\,\mu_{P'}=\mu_P\qquadd\text{for all }P\preceq P'.
\tag{9.5}
\]

Hence \${\mu\_P}\_{P\in\mathscr P}\$ is a **projective family**. Tightness follows from uniform Gårding/coercivity (Ch. 4), giving **Kolmogorov extension**:
...
Time-slice the partition \$P\$ into levels \${t\_k}\$. The coarse-graining (9.4) can be implemented via **Markov transfer kernels** \$K\_{k\to k+1}(x\_{t\_k},\mathrm d x\_{t\_{k+1}})\$ induced by the local quadratic principal symbol, with **finite-speed** propagation constant \$v\_\ast\$ determined by the single-cone bounds. The corresponding Dobrushin influence coefficients \$\alpha(A\to B)\$ between spatial blocks \$A\$ at \$t\_k\$ and \$B\$ at \$t\_{k+1}\$ obey
...
**Definition 10.1 (Reproduction number).** $\mathcal R_{\rm coh}(\ell,\Delta t)=\eta_{\rm align}(\Delta t)\,(1-L(\ell,\Delta t))\,M(\ell,\Delta t).$
...
Thus $\mathcal R_{\rm coh}>1$ if $(1-e^{-\gamma_W\ell/v_{\rm LR}})C_M>(1-C_L\alpha_{\rm leak}/v_{\rm LR})^{-1}.$
...
$\delta\mathcal S=\tfrac12\int (T^{\rm eff}_{\mu\nu})\,\delta g^{\mu\nu}\,\sqrt{-g}\,d^4x,\qquadd T^{\rm eff}=T^{\rm vis}+T^{\rm fast}.$
...
$V(\theta)=\Lambda_{\rm cx}^4\,(1-\cos\theta),\qquadd \Lambda_{\rm cx}^4\propto\alpha_{\rm cx},$
